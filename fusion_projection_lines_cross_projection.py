...
def _create_vertical_line(P1, P2, img):
    ...
    # predefine numpy array for output based on distance between points
    itbuffer = np.empty(shape=(int(dYa)+int(dXa), 2), dtype=np.float32)
    itbuffer.fill(np.nan)
    # vertical line segment
    itbuffer[:dYa,0] = int(P1[0])
    if P1_y > P2_y:
        itbuffer[:dYa,1] = np.arange(P1_y-1, P1_y - dYa -1, -1)
    else:
        itbuffer[:dYa,1] = np.arange(P1_y+1, P1_y+dYa+1)
    
    #horizontal line segment
    itbuffer[dYa:,1] = int(itbuffer[math.floor(dYa/2),1])
    if (dXa % 2) == 0:#even
        itbuffer[dYa:,0] = np.arange(P1[0]-dXa/2, P1[0]+dXa/2)
    else:#odd
        itbuffer[dYa:,0] = np.arange(P1[0]-math.floor(dXa/2), P1[0]+math.floor(dXa/2)+1)
